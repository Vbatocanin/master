...
from api.modeling import deeplab, deeplab_masks
from pytorch_networks.caustics.backbone import UNet
...
class InferenceCaustics:
    def __init__(self, causticsWeightsFile, imgHeight, imgWidth):
        """Class to run Inference of the Caustics Prediction Model

        Args:
            causticsWeightsFile (str): Path to the weights file for the model
            segmentationModel (str): Which model to use. Can be one of ["deeplab_xception", "deeplab_resnet", "drn"]
            imgHeight (int): The height to which images should be resized to before passing to the model.
            imgWidth (int): The width to which images should be resized to before passing to the model.
        """
        ...
        # Load Checkpoint and its data (weights file)
        if not os.path.isfile(causticsWeightsFile):
            raise ValueError(
                'Invalid path to the given weights file in config. The file "{}" does not exist'.format(
                    causticsWeightsFile))

        self.model.load_state_dict(torch.load(causticsWeightsFile))
        self.model.eval()
    ...
    def segmentCausticsOnNumpyImage(self, img, dilate_mask=False, inverse_mask=False):
        """Runs inference of caustics prediction model on a Numpy Image
        ...
        Args:
            img (numpy.ndarray): shape=(H, W, 3)
                                 ...
                                 if dtype=np.uint8, Expected data in range (0, 255)
                                 Obtained normally by using PIL to open image (in mode RGB) and converting to numpy
            dilate_mask (bool): Mask dilation switch
...
def subtract_caustics_from_mask(original_image: np.array,
                                raw_segmentation: np.array,
                                caustics_segmentation_model: InferenceCaustics):

    caustics_inverse_mask = caustics_segmentation_model.segmentCausticsOnNumpyImage(original_image,
                                                                                    inverse_mask=True)
    caustics_inverse_mask = cv2.resize(caustics_inverse_mask,raw_segmentation.shape,interpolation=cv2.INTER_NEAREST)

    return caustics_inverse_mask * raw_segmentation

...
import scipy
import torch
from api import inference_models
from api.utils import exr_saver, _get_point_cloud, write_point_cloud, depth2rgb, exr_loader, scale_depth, \
    save_uint16_png, unscale_depth
...
class DepthToDepthCompletion(object):
    ...
    Attributes:
        ...
        causticsWeightsFile (str): Path to the weights file for caustics prediction model.
    ...
    def __init__(
        ...
        if masksWeightsFile:
            ...
            self.inferenceCaustics = inference_models.InferenceCaustics(causticsWeightsFile=causticsWeightsFile,
                                                                        imgHeight=outlinesInferenceHeight,
                                                                        imgWidth=outlinesInferenceWidth)
    ...
    def store_depth_completion_outputs(self, root_dir, files_prefix, min_depth=0.0, max_depth=1.5):
        ...
        exr_saver(gt_depth_filename, self.depth_gt, ndim=3)
        ...
        xyz_points, rgb_points = _get_point_cloud(self.input_image, self.depth_gt, self.fx, self.fy, self.cx,
                                                  self.cy)
        ...
        exr_saver(input_depth_filename, self.orig_input_depth, ndim=3)
        ...
        # write_point_cloud(input_ptcloud_filename, self.input_image, self.orig_input_depth, self.fx, self.fy,
        #                         self.cx, self.cy)
        xyz_points, rgb_points = _get_point_cloud(self.input_image, self.orig_input_depth, self.fx, self.fy,
                                                  self.cx, self.cy)
        ...
        exr_saver(input_depth_filename, self.input_depth, ndim=3)
        ...
        write_point_cloud(input_ptcloud_filename, self.input_image, self.input_depth, self.fx, self.fy, self.cx,
                          self.cy)
        ...
        exr_saver(output_depth_filename, self.output_depth, ndim=3)
        ...
        write_point_cloud(output_ptcloud_filename, self.surface_normals_rgb, self.output_depth, self.fx, self.fy,
                          self.cx, self.cy)
        ...
        input_depth_rgb = depth2rgb(self.input_depth,
                                    min_depth=min_depth,
                                    max_depth=max_depth,
                                    color_mode=COLOR_MAP)
        orig_input_depth_rgb = depth2rgb(self.orig_input_depth,
                                         min_depth=min_depth,
                                         max_depth=max_depth,
                                         color_mode=COLOR_MAP)
        output_depth_rgb = depth2rgb(self.output_depth,
                                     min_depth=min_depth,
                                     max_depth=max_depth,
                                     color_mode=COLOR_MAP)
        gt_depth_rgb = depth2rgb(self.depth_gt, min_depth=min_depth, max_depth=max_depth, color_mode=COLOR_MAP)
        ...
        mask_valid_region_3d = np.stack([self.mask_valid_region] * 3,
                                        axis=2)  # Ground truth mask - invalid depth pixels
    ...
    def _modify_synthetic_depth_delete_objects(self, input_depth, json_file_path, variant_mask_path, centroid_width=5):
        ...
        """
        # Load variant mask
        variant_mask = exr_loader(variant_mask_path, ndim=1)
    ...
    def _modify_depth_delete_masks(self, rgb_image, input_depth, dilate_mask=True):
        ...
        # Run caustics inference to get caustic segmentation mask
        self.mask_predicted = inference_models.subtract_caustics_from_mask(rgb_image,
                                                                           self.mask_predicted,
                                                                           self.inferenceCaustics)

        self.mask_predicted = cv2.resize(self.mask_predicted,
                                         (self.outputImgWidth, self.outputImgHeight),
                                         interpolation=cv2.INTER_NEAREST)
    ...
    def _complete_depth(self, input_image, input_depth, inertia_weight=1000, smoothness_weight=0.001, tangent_weight=1):
        ...
        scaled_input_depth = scale_depth(input_depth)
        save_uint16_png(self._PATH_TMP_INPUT_DEPTH, scaled_input_depth)
        ...
        save_uint16_png(self._PATH_TMP_OCCLUSION_WEIGHTS, self.occlusion_weight)
        ...
        output_depth = unscale_depth(scaled_output_depth)  # output depth in meters

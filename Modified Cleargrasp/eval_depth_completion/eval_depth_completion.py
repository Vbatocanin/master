...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--maskInputDepth', action="store_true",
    ...
    #         camera_params = AttrDict(config_intr_yaml)
    ...
    depthcomplete = depth_completion_api.DepthToDepthCompletion(normalsWeightsFile=config.normals.pathWeightsFile,
                                                                ...
                                                                causticsWeightsFile=config.caustics.causticsWeightsFile,
    ...
    for i in range(len(rgb_file_list)):
        ...
        # NOTE: If no gt_depth present, it means the depth itself is gt_depth (syn data). We mask out all objects in
        # input depth so depthcomplete can't cheat.
        ...
        # If Ground Truth depth folder is given, use that to compute errors. In case of Synthetic data, input depth
        # is GT depth.
